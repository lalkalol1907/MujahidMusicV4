...
class InteractionListener(
    ...
    override fun onReady(event: ReadyEvent) {
        ...
        if (devGuildId == null) {
            event.jda.updateCommands().addCommands(commands).queue { success ->
                log.info("Registered {} commands globally", commands.size)
            }
            return
        }

        val guild: Guild? = event.jda.getGuildById(devGuildId)
        if (guild != null) {
    ...
    override fun onButtonInteraction(event: ButtonInteractionEvent) {
        MusicButtonHandler.handle(event, lavalink)
    }
    ...
    override fun onGuildVoiceUpdate(event: GuildVoiceUpdateEvent) {
        ...
        if (event.channelLeft == null || event.channelLeft!!.idLong != selfChannel.idLong) {
            return
        }
